...
import {  FaTimesCircle, FaBoxes, FaHome, FaBoxOpen,  FaExchangeAlt, FaExclamationTriangle, FaClipboardCheck, FaPowerOff, FaCalendarAlt, FaTruckLoading, FaClipboardList, FaWarehouse } from "react-icons/fa";
...
function Home() {
 ...
 const HomeNavigate = () => navigate('/home');
  ...
  const handleCardClick = (index) => {
    setActiveCard(index === activeCard ? null : index);s
  };

  const cards = [
    { icon: <FaBoxes className="text-success display-6 mb-2" />, title: "Total Product Inventory", value: 100, path: "/products" },
    { icon: <FaTimesCircle className="text-danger display-6 mb-2" />, title: "Out of Stock", value: 4, path: "/outstock" },
    ...
    { icon: <FaTruckLoading className="text-success display-6 mb-2" />, title: "Incoming Supplies", value: 21, path: "/incoming-supplies" },
  ];
                    ...
                    <div key={index} className="col">
                        ...
                        >
                            <div className="card-body text-center" style={{ padding: '1rem' }}>
                                {cardData.icon}
                                <h5 className="card-title" style={{ fontSize: '1.2rem', marginBottom: '0.5rem', color: '#002d62' }}>
                                    {cardData.title}
                                </h5>
                                <p className="card-text fs-4 fw-bold" style={{ color: '#333' }}>
                                    {cardData.value}
                                </p>
                            </div>
                        </div>
